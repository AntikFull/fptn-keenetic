...
<details>
  <summary><strong>Установка и настройка FPTN на Keenetic (Entware)</strong></summary>

Вы можете установить FPTN-клиент с удобной веб-панелью управления на домашний роутер Keenetic с помощью интерактивного скрипта.
...
1. Установит нужные пакеты (веб-сервер `lighttpd`, `php8-cgi`, `curl` и др.).
2. Определит архитектуру роутера (`aarch64`, `armv7` или `mipsel`) и скачает подходящий статический бинарник клиента.
3. Попросит ввести имя интерфейса, порт для веб-панели (по умолчанию `8088`) и токен подписки.
4. Создаст и настроит TUN-интерфейс в KeeneticOS (с флагами `ip global` и подстройкой TCP MSS), чтобы роутер видел его как рабочее интернет-подключение.
5. Настроит автозапуск службы и веб-сервер.

После завершения установки откройте веб-панель по адресу:
`http://192.168.1.1:8088/fptn/`

[Подробные инструкции по настройке и маршрутизации на Keenetic](deploy/keenetic/README.md).
</details>
